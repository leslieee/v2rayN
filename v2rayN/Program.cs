...
namespace v2rayN
{
    static class Program
    {
        ...
        private static string autoRunName = "v2rayN";
        ...
        public static void SetAutoStart(bool enable)
        {
            try
            {
                using (var regKey = Registry.CurrentUser.CreateSubKey(autoRunRegPath))
                {
                    if (enable)
                        regKey.SetValue(autoRunName, Application.ExecutablePath);
                    else
